...
const replacementItem: Ref<FirebaseTemplateItem> = ref(
  new FirebaseTemplateItem("", "", "", "")
);
const selectedItem: Ref<FirebaseTemplateItem> = ref(
  new FirebaseTemplateItem("", "", "", "")
);
...
watch(
  ...
  (key) => {
    ...
    replacementItem.value = new FirebaseTemplateItem(
      item.key,
      item.singular,
      item.plural,
      item.type
...
async function editSelectedKey(): Promise<void> {
  ...
  try {
    await state.DATABASE_UPDATE_FIREBASE_TEMPLATE_ITEM(
      props.selectedKey,
...
<template>
  <yes-no-modal
    ...
    @close="emit('toggleEditModal', false)"
    @no="emit('toggleEditModal', false)"
    @yes="editSelectedKey"
    cancelText="Cancel"
    confirmText="Done"
  ...
  >
    <template #title
      >Edit template-item: "{{ selectedItem!.singular }}"?</template
    >
    <template #body>
      <form @submit.prevent="editSelectedKey">
        <input-validate
          v-model:value="replacementItem.singular"
          :title="labelMessage(selectedItem!.singular)"
        />
        <input-validate
          v-model:value="replacementItem.plural"
          :title="labelMessage(selectedItem!.plural)"
        />
        <input-validate
          v-model:value="replacementItem.type"
          :title="labelMessage(selectedItem!.type)"
        />
      </form>
    </template>
  </yes-no-modal>
