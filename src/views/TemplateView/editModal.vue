...
const replacementItem: Ref<FirebaseTemplate> = ref(
  new FirebaseTemplate("", "")
);
...
async function editSelectedKey(): Promise<void> {
  ...
  try {
    await state.DATABASE_UPDATE_FIREBASE_TEMPLATE(
      props.selectedKey,
      replacementItem.value.value
    );
    emit("submit", true);
  } catch (err) {
...
<template>
  <yes-no-modal
    ...
    confirmText="Done"
    :confirmDisabled="
      replacementItem.value === selectedItem.value || v$.$invalid
    "
  >
    ...
    <template #body>
      ...
      />
      <custom-text-area
        v-model:value="replacementItem.value"
        title="New"
        labelLength="3rem"
      />
    </template>
